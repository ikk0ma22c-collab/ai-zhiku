...
from src.import_processor.exceptions import StateFieldError, MilvusError
...
class NodeImportMilvus(BaseNode):
    ...
    def _step_1_check_input(self, state: ImportGraphState) -> Tuple[List[Dict[str, Any]], int]:
        """
        步骤 1: 输入数据有效性校验
        功能：
            验证chunks数据格式是否正确，确保核心字段完整
            提取向量维度信息，为后续入库做准备
        参数：
            state: 工作流状态对象，包含chunks数据
        返回：
            Tuple[List[Dict[str, Any]], int]: 校验通过的chunks列表+向量维度
        """
    ...
    def _step_2_prepare_collection(self, vector_dimension: int):
        ...
        # 2. 集合不存在则创建
        collections_name = os.getenv("CHUNKS_COLLECTION_NAME")
